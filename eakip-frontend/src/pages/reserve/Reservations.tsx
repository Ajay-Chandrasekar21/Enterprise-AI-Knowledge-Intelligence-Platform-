...
export const Reservations: React.FC = () => {
  ...
  const columns = [
    { header: 'Reserved Book', accessor: (row: ReservationItem) => (
      <div class="flex items-center space-x-3">
        ...
        <span class="font-medium text-slate-800 dark:text-slate-200">{row.bookTitle}</span>
      </div>
    ) },
    ...
    { header: 'Queue Status', accessor: (row: ReservationItem) => {
      ...
      return (
        <div class="flex items-center space-x-2">
          <span class={`px-2.5 py-1 rounded-md text-xs font-semibold uppercase ${styles[row.status]}`}>{row.status}</span>
          {row.status === 'PENDING' && (
            <span class="text-xs text-slate-400 font-semibold">(Queue Position #{row.queuePosition})</span>
          )}
        </div>
      );
    } },
  ...
  return (
    <div class="space-y-6">
      <div>
        <h1 class="text-2xl font-bold">My Holds Queue</h1>
        <p class="text-slate-500 text-sm">Review active reservations queue placement and cancel pending bookings</p>
      </div>
